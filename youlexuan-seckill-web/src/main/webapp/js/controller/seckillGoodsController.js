...
app.controller('seckillGoodsController' ,function($scope,$controller,$location,$interval   ,seckillGoodsService,seckillOrderService){
            ...
            function(response){
                ...
                var time = $interval(function () {
                    if ($scope.seconds>0){
                        ...
                        $scope.timeStr = coverTimeStr($scope.seconds);
                    } else {
     ...
     * @param seconds
     */
    coverTimeStr=function (allseconds) {
        var days = Math.floor(allseconds/(60*60*24));//天
        var hours = Math.floor((allseconds - days*60*60*24)/(60*60));//小时
        var minutes = Math.floor((allseconds - days*60*60*24 - hours*60*60)/60);//分钟
        var seconds = allseconds - days*60*60*24 - hours*60*60 - minutes*60;//秒
        var timeString = "";
        if (days>0){
            timeString = days+"天";
        }
