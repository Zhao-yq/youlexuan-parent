...
public class OrderServiceImpl implements OrderService {
	...
	/**
	 * 增加
	 * 将cartList中的信息转成order相关信息
	 * 1、包括cart基本信息入到tb_order表中
	 * 2、cart中的购物项信息入到tb_order_item表中
	 * 部分数据从前台获取到的，eg：付款方式，收货人地址信息等
	 */
	@Override
	public void add(TbOrder order) {
		...
		double totalFee = 0.0;//总金额 （元）
		for (Cart cart:cartList){
			long orderId = idWorker.nextId();//分布式id
			...
			tborder.setCreateTime(order.getCreateTime());//订单创建日期
			tborder.setUpdateTime(order.getUpdateTime());//订单更新日期
		...
		if("1".equals(order.getPaymentType())){
			...
			payLog.setOutTradeNo(idWorker.nextId()+"");//要求交易号必须唯一
			payLog.setPayType("1");
